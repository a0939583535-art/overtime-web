...
// ✅ 修改：多一個 manualDayType 參數（預設 auto）
function upsertRecord(dateStr, startStr, endStr, editId, manualDayType = "auto") {
  ...
  // ✅ 修改：支援手動指定類型（auto 則照原本計算）
  const dType = (manualDayType === "auto")
...
function addRecord() {
  ...
  // 新增維持 auto
  const res = upsertRecord(dateStr, startStr, endStr, null, "auto");
  if (!res.ok) {
    alert(res.error);
    return;
  }
...
function openEditModal(id) {
  ...
  editHintEl.textContent = "調整日期、時間與類型後按「儲存」。會重新計算時數與加發金額。";
...
function saveEditModal() {
  ...
  // ✅ 新增：讀取手動類型
  const manual = editDayTypeEl ? (editDayTypeEl.value || "auto") : "auto";

  const res = upsertRecord(newDate, newStart, newEnd, editingRecordId, manual);
  if (!res.ok) {
    alert(res.error);
    return;
  }
...
function bulkAdd() {
  ...
  for (const dateStr of targetDates) {
    // 批量新增維持 auto
    const res = upsertRecord(dateStr, startTimeStr, endTimeStr, null, "auto");
    if (!res.ok) {
